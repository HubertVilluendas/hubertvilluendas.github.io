...
- **TD1** 
    ...
    >         & + (2m-1) + (2m+3) + \cdots + (2(n-k+1)-1) + \cdots + 3 + 1\\
    >       = & \underbrace{2m + 2m + \cdots + 2m + \cdots + 2m}_{m\text{ terms}}\\
            = 2m^2.
    > \end{align*}\\)
    > Therefore, \\(S = m^2\\).

    > **Proposition** : for all \\(n\in\mathbb{N}\\), \\(\Delta_n^2=\sum^{\Delta_n}_{k=1}(2k-1)=\sum^n_{k=1}k^3\\).
